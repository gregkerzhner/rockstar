angular.module('rockstar.dashboard.user-climb', [
  ...
  function ($stateProvider) {
    $stateProvider.state('rockstar.dashboard-container.dashboard.user-climb', {
  ...
  function ($scope, $stateParams, userClimbs, attempts, currentUser) {
    ...
    attempts.index($stateParams.user_climb_id).then(function (attempts) {
      ...
      for (var i = 0; i < $scope.attempts.length; i++) {
        $scope.attempts[i].date = new Date($scope.attempts[i].startTime).toDateString() + ' ' + new Date($scope.attempts[i].startTime).getHours() + ':' + new Date($scope.attempts[i].startTime).getMinutes();
